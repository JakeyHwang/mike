...
| --- | --- | --- |
| Inference | Default models are the gateway's (`ollama/qwen/qwen3.8-27b` for chat, titles and tabular review; titles run with reasoning switched off via vLLM's `chat_template_kwargs`, since the gateway's models otherwise spend a short completion's whole budget on hidden reasoning). The gateway is reached through the existing Ollama provider: `OLLAMA_BASE_URL` + `OLLAMA_API_KEY`. Speech and embedding models are hidden from the picker. | `backend/src/lib/llm/models.ts`, `backend/src/lib/llm/providers.ts`, `backend/src/lib/chatTitle.ts`, `backend/src/routes/models.ts`, `frontend/src/app/components/assistant/ModelToggle.tsx`, `frontend/src/app/hooks/useSelectedModel.ts`, `word-addin/src/taskpane/lib/modelCatalog.ts` |
